...
function getPedido($id_pedido) {
    ...
    $sql = "SELECT p.id_pedido, p.curso, p.retirante, p.fecha_pedido, p.estado, p.fecha_devolucion, 
            ...
            WHERE p.id_pedido = ? AND p.estado = 'prestado'";
...
function procesarDevolucion($id_pedido, $observaciones) {
    $conn = connectDB();
    ...
    try {
        ...
        $stmt = $conn->prepare("UPDATE pedidos SET estado = 'devuelto', fecha_devolucion = CURRENT_TIMESTAMP, observaciones = ? WHERE id_pedido = ?");
        $stmt->bind_param("si", $observaciones, $id_pedido);
        $stmt->execute();
        
        // Obtener las herramientas del pedido
        $stmt = $conn->prepare("SELECT herramienta, cantidad FROM detalle_pedido WHERE id_pedido = ?");
        $stmt->bind_param("i", $id_pedido);
        $stmt->execute();
        $result = $stmt->get_result();
        
        // Actualizar el inventario (devolver las herramientas)
        while($row = $result->fetch_assoc()) {
            $herramienta_id = $row['herramienta'];
            $cantidad = $row['cantidad'];
            
            $stmt = $conn->prepare("UPDATE herramientas SET cantidad = cantidad + ? WHERE codigo = ?");
            $stmt->bind_param("ii", $cantidad, $herramienta_id);
            $stmt->execute();
        }
...
// Si no se encuentra el pedido o ya está devuelto, redirigir
if (!$pedido) {
    header("Location: pedidos_lista.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $resultado = procesarDevolucion($id_pedido, $observaciones);
    
    if ($resultado === true) {
        // Redireccionar a la lista de pedidos
        header("Location: pedidos_lista.php?devuelto=1");
        exit();
    } else {
        $mensaje = "Error al procesar la devolución: " . $resultado;
        $tipo_mensaje = "danger";
    }
}
...
<head>
    ...
    <title>Registrar Devolución - Sistema de Pañol</title>
...
<body>
    ...
    <div class="container my-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1>Registrar Devolución de Pedido #<?php echo $id_pedido; ?></h1>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="mb-3 row">
                            <label class="col-sm-4 col-form-label fw-bold">Fecha de pedido:</label>
                            <div class="col-sm-8">
                ...
                <form method="POST" action="">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="observaciones" name="observaciones" rows="4" 
                                  placeholder="Registre cualquier observación sobre el estado de las herramientas devueltas, incidencias, etc."><?php echo isset($_POST['observaciones']) ? htmlspecialchars($_POST['observaciones']) : ''; ?></textarea>
                        <div class="form-text">
                            Puede dejar constancia de cualquier incidencia, deterioro o comentario sobre las herramientas devueltas.
                        </div>
                    </div>
